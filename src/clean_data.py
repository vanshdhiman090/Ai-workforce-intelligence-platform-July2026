...
def run_pipeline():
    """
    Full data cleaning pipeline: load raw data, remove duplicates,
    handle missing values, flag salary coverage, extract skills,
    ...
    before = len(df)
    df_clean = df.drop_duplicates(subset=["title", "company", "location"]).copy()
    after = len(df_clean)
    ...
    df_clean["extracted_skills"] = df_clean["description"].apply(extract_skills)
